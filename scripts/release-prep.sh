#!/usr/bin/env bash
# release-prep — bump the workspace version and rotate CHANGELOG [Unreleased] block
#
...
# Bump the workspace version line (Cargo.toml's [workspace.package].version is
# the only top-level 'version = ...' line; member crates use
# version.workspace = true). Use sed -i.bak for portability between GNU and
...
# version bump or CHANGELOG rotation that breaks something only surfaces in CI
# after push — wasting a feedback cycle. set -e propagates the failure; the
# maintainer fixes the issue against the bumped tree and either re-runs the
# script or commits. Skipped in environments without a justfile.
